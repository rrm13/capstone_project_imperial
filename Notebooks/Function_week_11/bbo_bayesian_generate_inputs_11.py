#!/usr/bin/env python3
"""
Bayesian Optimization generator for BBO Challenge - Round 11 input queries.

This script uses:
  - inputs_11.txt  : historical input query rows. If this file already contains
                     an 11th row, the script uses only the first len(outputs_10.txt)
                     rows for training and prints the existing row for comparison.
  - outputs_10.txt : objective values/scores for the first 10 rounds.

It writes:
  - inputs_11_generated.txt : one generated row of 8 query vectors for round 11.
...
# Configuration
# -----------------------------
INPUTS_FILE = Path("inputs_11.txt")
OUTPUTS_FILE = Path("outputs_10.txt")
GENERATED_FILE = Path("inputs_11_generated.txt")
...
# Candidate budget. Increase these if you want a more expensive/acquisitive search.
N_GLOBAL_CANDIDATES = 160_000
N_LOCAL_CANDIDATES = 55_000
N_BOUNDARY_CANDIDATES_PER_DIM = 450
XI = 0.01
...
def propose_for_task(X: np.ndarray, y: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    # 2) Local exploitation around the current best observed vector.
    best_idx = int(np.argmax(y) if MAXIMIZE else np.argmin(y))
    best_x = X[best_idx]
    local_candidates = best_x + rng.normal(0.0, 0.045, size=(N_LOCAL_CANDIDATES, dim))
    local_candidates = np.clip(local_candidates, low, high)

    # 3) Boundary-aware probes. Some black-box functions peak near 0 or 1.
    n_boundary = max(1500, N_BOUNDARY_CANDIDATES_PER_DIM * dim)
    boundary_candidates = rng.uniform(low, high, size=(n_boundary, dim))
    boundary_mask = rng.random(boundary_candidates.shape) < 0.24
    ...
    if MAXIMIZE:
        top_indices = np.argsort(y)[-min(4, len(y)):]
    else:
        top_indices = np.argsort(y)[:min(4, len(y))]

    elite_candidates = []
    for idx in top_indices:
        elite = X[idx] + rng.normal(0.0, 0.028, size=(max(3000, 800 * dim), dim))
...
def main() -> None:
    ...
    if len(input_rows) > n_training_rounds:
        print("Existing row in inputs_11.txt after the training rows, for comparison:")
