#!/usr/bin/env python3
"""
Bayesian Optimization generator for BBO Challenge - Round 8 input queries.

This script uses:
  - inputs_8.txt   : historical input query rows. If this file already contains
                     an 8th row, the script uses only the first len(outputs_7.txt)
                     rows for training and prints the existing row for comparison.
  - outputs_7.txt  : objective values/scores for the first 7 rounds.

It writes:
  - inputs_8_generated.txt : one generated row of 8 query vectors for round 8.
...
# Configuration
# -----------------------------
INPUTS_FILE = Path("inputs_8.txt")
OUTPUTS_FILE = Path("outputs_7.txt")
GENERATED_FILE = Path("inputs_8_generated.txt")
...
N_GLOBAL_CANDIDATES = 100_000
N_LOCAL_CANDIDATES = 30_000
N_BOUNDARY_CANDIDATES_PER_DIM = 300
XI = 0.01
...
def safe_eval_numpy_repr(text: str):
    ...
    # Convert np.float64(1.23) into 1.23 for cleaner evaluation.
...
def propose_for_task(X: np.ndarray, y: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    best_x = X[best_idx]
    local_candidates = best_x + rng.normal(0.0, 0.06, size=(N_LOCAL_CANDIDATES, dim))
    local_candidates = np.clip(local_candidates, low, high)

    # 3) Boundary-aware probes. Some BBO functions have optima near 0 or 1.
    n_boundary = max(1000, N_BOUNDARY_CANDIDATES_PER_DIM * dim)
    boundary_candidates = rng.uniform(low, high, size=(n_boundary, dim))
    boundary_mask = rng.random(boundary_candidates.shape) < 0.18
...
def main() -> None:
    ...
    if len(input_rows) > n_training_rounds:
        print("Existing row in inputs_8.txt after the training rows, for comparison:")
