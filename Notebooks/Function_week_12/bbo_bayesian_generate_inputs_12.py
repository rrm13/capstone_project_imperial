#!/usr/bin/env python3
"""
Bayesian Optimization generator for BBO Challenge - Round 12 input queries.

This script uses:
  - inputs_12.txt  : historical input query rows. If this file already contains
                     a 12th row, the script uses only the first len(outputs_11.txt)
                     rows for training and prints the existing row for comparison.
  - outputs_11.txt : objective values/scores for the first 11 rounds.

It writes:
  - inputs_12_generated.txt : one generated row of 8 query vectors for round 12.
...
# Configuration
# -----------------------------
INPUTS_FILE = Path("inputs_12.txt")
OUTPUTS_FILE = Path("outputs_11.txt")
GENERATED_FILE = Path("inputs_12_generated.txt")
...
# Candidate budget. Increase for more expensive search; decrease for faster runtime.
N_GLOBAL_CANDIDATES = 180_000
N_LOCAL_CANDIDATES = 65_000
N_BOUNDARY_CANDIDATES_PER_DIM = 500
XI = 0.01
...
def propose_for_task(X: np.ndarray, y: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    best_x = X[best_idx]
    local_candidates = best_x + rng.normal(0.0, 0.040, size=(N_LOCAL_CANDIDATES, dim))
    local_candidates = np.clip(local_candidates, low, high)

    # 3) Boundary-aware probes. Some black-box functions peak near 0 or 1.
    n_boundary = max(2000, N_BOUNDARY_CANDIDATES_PER_DIM * dim)
    boundary_candidates = rng.uniform(low, high, size=(n_boundary, dim))
    boundary_mask = rng.random(boundary_candidates.shape) < 0.26
    ...
    for idx in top_indices:
        elite = X[idx] + rng.normal(0.0, 0.025, size=(max(3500, 950 * dim), dim))
...
def main() -> None:
    ...
    if len(input_rows) > n_training_rounds:
        print("Existing row in inputs_12.txt after the training rows, for comparison:")
