#!/usr/bin/env python3
"""
Bayesian Optimization generator for BBO Challenge - Round 10 input queries.

This script uses:
  - inputs_10.txt  : historical input query rows. If this file already contains
                     a 10th row, the script uses only the first len(outputs_9.txt)
                     rows for training and prints the existing row for comparison.
  - outputs_9.txt  : objective values/scores for the first 9 rounds.

It writes:
  - inputs_10_generated.txt : one generated row of 8 query vectors for round 10.
...
# Configuration
# -----------------------------
INPUTS_FILE = Path("inputs_10.txt")
OUTPUTS_FILE = Path("outputs_9.txt")
GENERATED_FILE = Path("inputs_10_generated.txt")
...
# Candidate budget. Increase these if you want more aggressive search quality.
N_GLOBAL_CANDIDATES = 140_000
N_LOCAL_CANDIDATES = 45_000
N_BOUNDARY_CANDIDATES_PER_DIM = 400
XI = 0.01
...
def propose_for_task(X: np.ndarray, y: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    best_x = X[best_idx]
    local_candidates = best_x + rng.normal(0.0, 0.050, size=(N_LOCAL_CANDIDATES, dim))
    local_candidates = np.clip(local_candidates, low, high)

    # 3) Boundary-aware probes. Some BBO functions peak near 0 or 1.
    n_boundary = max(1200, N_BOUNDARY_CANDIDATES_PER_DIM * dim)
    boundary_candidates = rng.uniform(low, high, size=(n_boundary, dim))
    boundary_mask = rng.random(boundary_candidates.shape) < 0.22
    boundary_candidates[boundary_mask] = rng.choice([low, high], size=boundary_mask.sum())

    # 4) Elite perturbation around top historical points.
    if MAXIMIZE:
    ...
    for idx in top_indices:
        elite = X[idx] + rng.normal(0.0, 0.030, size=(max(2500, 650 * dim), dim))
...
def main() -> None:
    ...
    if len(input_rows) > n_training_rounds:
        print("Existing row in inputs_10.txt after the training rows, for comparison:")
