#!/usr/bin/env python3
"""
Bayesian Optimization generator for BBO Challenge - Round 13 input queries.

This script uses:
  - inputs_13.txt  : historical input query rows. If this file already contains
                     a 13th row, the script uses only the first len(outputs_12.txt)
                     rows for training and prints the existing row for comparison.
  - outputs_12.txt : objective values/scores for the first 12 rounds.

It writes:
  - inputs_13_generated.txt : one generated row of 8 query vectors for round 13.
...
# Configuration
# -----------------------------
INPUTS_FILE = Path("inputs_13.txt")
OUTPUTS_FILE = Path("outputs_12.txt")
GENERATED_FILE = Path("inputs_13_generated.txt")
...
# Candidate budget. Increase for higher search effort; decrease for faster runtime.
N_GLOBAL_CANDIDATES = 200_000
N_LOCAL_CANDIDATES = 75_000
N_BOUNDARY_CANDIDATES_PER_DIM = 550
XI = 0.01
...
# -----------------------------
def extract_top_level_lists(text: str) -> List[str]:
    """
    Extract top-level list records from text.

    This is robust to:
      - wrapped numpy arrays split across physical lines
      - records accidentally concatenated like: ][array(...)]
    """
    records: List[str] = []
    balance = 0
    start = None
    ...
    for i, ch in enumerate(text):
        if ch == "[":
            ...
            raise ValueError("Unexpected closing bracket while parsing file.")

    if balance != 0:
        raise ValueError("Unbalanced brackets while parsing file.")
...
def load_inputs(path: Path) -> List[List[np.ndarray]]:
    ...
    for record in extract_top_level_lists(path.read_text()):
...
def propose_for_task(X: np.ndarray, y: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    # 1) Global exploration across the full [0, 1]^d domain.
    global_candidates = rng.uniform(low, high, size=(N_GLOBAL_CANDIDATES, dim))

    # 2) Exploit around the current best observed vector.
    best_idx = int(np.argmax(y) if MAXIMIZE else np.argmin(y))
    best_x = X[best_idx]
    local_candidates = best_x + rng.normal(0.0, 0.038, size=(N_LOCAL_CANDIDATES, dim))
    local_candidates = np.clip(local_candidates, low, high)

    # 3) Boundary-aware probes. Some black-box functions peak near 0 or 1.
    n_boundary = max(2500, N_BOUNDARY_CANDIDATES_PER_DIM * dim)
    boundary_candidates = rng.uniform(low, high, size=(n_boundary, dim))
    boundary_mask = rng.random(boundary_candidates.shape) < 0.28
    boundary_candidates[boundary_mask] = rng.choice([low, high], size=boundary_mask.sum())

    # 4) Elite perturbation around top historical points for exploitation.
    if MAXIMIZE:
        top_indices = np.argsort(y)[-min(5, len(y)):]
    else:
        top_indices = np.argsort(y)[:min(5, len(y))]

    elite_candidates = []
    for idx in top_indices:
        elite = X[idx] + rng.normal(0.0, 0.022, size=(max(4000, 1100 * dim), dim))
...
def main() -> None:
    ...
    if len(input_rows) > n_training_rounds:
        print("Existing row in inputs_13.txt after the training rows, for comparison:")
